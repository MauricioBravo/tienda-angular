...
const registro_admin = async function (req, res) {
    ...
    var admins_arr = [];
    admins_arr = await Admin.find({ email: data.email }); //funcion buscar en modelo Admin

    if (admins_arr.length == 0) { //si el largo del mail es cero no existe por lo que lo registra
        //REGISTRO
...
const login_admin = async function (req, res) {
    var data = req.body;
    var admin_arr = [];
    admin_arr = await Admin.find({email:data.email }); //guarda en email la data del body req

    if (admin_arr.length == 0) { //no hay ningun user registrado
